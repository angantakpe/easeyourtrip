...
def crop_image(folder_path, image):  # do not add "/" at the end in path
    try:
        ...
        if not os.path.exists(img_path):
            ...
            for alt_ext in [".jpg", ".jpeg", ".png"]:
                ...
                if os.path.exists(alt_path):
                    img_path = alt_path
                    image = os.path.basename(alt_path)
                    break
        ...
        )  # Adaptive Threshold - Gaussian Weighted Sum
        ...
        result2 = original_img[
            (y - 4 if y >= 4 else y) : (y + h + 4 if y + h + 4 <= maxh else y + h),
            (x - 4 if x >= 4 else x) : (x + w + 4 if x + w + 4 <= maxw else x + w),
        ]
        return result2
    except Exception as e:
...
def preprocess_image(
    folder_path, image, destination_folder_path=None
):  # do not add "/" at the end in path
    try:
        # converting img format to png
        destination_folder_path = (
            folder_path if destination_folder_path == None else destination_folder_path
        )
        if image is not None:
            ...
            # If we still can't find the file, raise an exception with clear error message
            if not os.path.exists(img_path):
                raise FileNotFoundError(f"Image file not found: {img_path}")
...
def rotate_bounding_box(bbox, angle, original_image_size, rotated_image_size):
    ...
    if xmin != None and ymin != None and xmax != None and ymax != None:
        ...
        bottom_right = (
            bottom_right[0] + center_shift_x,
            bottom_right[1] + center_shift_y,
        )
...
def rotate_coordinates(
    word_locations, angle, original_image_shape, rotated_image_shape, request_id
):
    ...
    try:
        ...
        rotated_image_center = (
            rotated_image_shape[0] / 2,
            rotated_image_shape[1] / 2,
        ...
        for word, coords_list in word_locations.items():
            ...
            for coords in coords_list:
                ...
                for i in range(0, len(coords), 2):  # Iterate over pairs (x, y)
                    ...
                    )  # Extend to keep the format [x1, y1, x2, y2, ...]

                rotated_coords.append(
                    rotated_set
                )  # Append the list of rotated coordinates
            rotated_locations[word] = rotated_coords

        return True, rotated_locations
    except Exception as e:
        debug_log(
            f"Exception in rotate_coordinates as {str(e)} ",
            "rotate_coordinates",
            request_id,
        )
...
def crop_passport(cropped_image_path, locs):
    try:
        ...
        if upper_coordinate:
            # Define boundaries
            pixcel_to_extend_horizontally, pixcel_to_extend_vertically = (
                get_crop_margin_from_text(
                    xmin_l, ymin_r, xmax_r, ymax_b, img_height, img_width
                )
            )
            y_start = int(
                max(ymin_r - int(pixcel_to_extend_vertically), 0)
            )  # Top boundary from "REPUBLIC"
            xmin_l = int(
                max(xmin_l - int(pixcel_to_extend_horizontally), 0)
            )  # Left boundary from first "<"
            xmax_r = int(
                min(xmax_r + int(pixcel_to_extend_horizontally), img_width)
            )  # Right boundary from last "<"
            ymax_b = int(
                min(ymax_b + int(pixcel_to_extend_vertically), img_height)
            )  # Bottom boundary from last "<"
...
def crop_img_text_locations(
    ...
    try:
        if document_type not in no_cropping:
            if document_type == "passport_front" and (
                "REPUBLIC" and "<" in text_string.upper()
            ):

                # pass_text_blob = get_passport_textblob(ocr_resp['text_blob'] , text_string)
                crop_paasport_status, crop_passport_path, x_top, y_top = crop_passport(
                    original_img_path, word_coordinates
                )
                if crop_paasport_status:
                    crop_pass_coord = adjust_coordinates_after_crop(
                        word_coordinates, x_top, y_top
                    )
                else:
                    crop_pass_coord = word_coordinates
                return (
                    Image.open(crop_passport_path),
                    crop_pass_coord,
                    list(crop_pass_coord.keys()),
                )
            ...
            pixcel_to_extend_horizontally, pixcel_to_extend_vertically = (
                get_crop_margin_from_text(
                    min_x, min_y, max_x, max_y, img_height, img_width
                )
            )
            min_y = int(
                max(min_y - int(pixcel_to_extend_vertically), 0)
            )  # Top boundary from "REPUBLIC"
            min_x = int(
                max(min_x - int(pixcel_to_extend_horizontally), 0)
            )  # Left boundary from first "<"
            max_x = int(
                min(max_x + int(pixcel_to_extend_horizontally), img_width)
            )  # Right boundary from last "<"
            ...
            cropped_img_coord = adjust_coordinates_after_crop(
                word_coordinates, min_x, min_y
            )
        ...
        debug_log(
            f"Exception in crop_img_text_locations as {str(e)} ",
...
def crop_and_generate_image_url(new_image_name, csv_row, sql_dict, jsonObj):
    ...
    try:
        ...
        try:  # preprocessing the image to crop the main object(document) from the document
            ...
            cropped_image_path = preprocess_image(
                folder_path, new_image_name, folder_path
            )
            end_pp = time.time()
            ...
            # Handle file extension issues in fallback case
            img_path = os.path.join(folder_path, new_image_name)
            ...
            if not os.path.exists(img_path):
                ...
                for alt_ext in ['.jpg', '.jpeg', '.png']:
                    ...
                    if os.path.exists(alt_path):
                        img_path = alt_path
                        new_image_name = os.path.basename(alt_path)
                        break
            
            # If we still can't find the file, log the error but continue with original path
            if not os.path.exists(img_path):
                print(f"Warning: Image file not found in fallback path: {img_path}")
