...
def process_page_threads(
    ...
    jsonObj["Time"][f"T_Time_Page_No_{i+1}"] = str(
        datetime.strptime(
            jsonObj["Time"][f"Processed_Page_{i+1}"], "%Y-%m-%d %H:%M:%S.%f"
        )
        - datetime.strptime(
            jsonObj["Time"][f"Started_Page_{i+1}"], "%Y-%m-%d %H:%M:%S.%f"
        )
    )
    ...
    sql_dict.update(
        {
            "final_time": end_pg,
            "total_page_time": end_pg - start_pg,
            "response_file": json_res_name,
            "page_status": "Success",
            "page_remark": "NO",
        }
    )
...
def docprocess(
    ...
    try:
        ...
        tempFilePaths = (
            []
        )  # making a array to collect the path of the temporary files stored on disc for later on deleting the files
        try:  # The standard json schema at the document level is predefined in assests/documents.json. Loading the schema for updating it.
        ...
        img_path, image_name, file_extension = save_and_gen_filepath(
            uploaded_filename, file_content, tempFilePaths
        )
        ...
        doccat = (
            set()
        )  # making a set of all unique documents in the file. later on convert it into list.
        ...
        if file_extension == "pdf":
            ...
            try:
                ...
                jsonObj["requestResponse"] = (
                    "Invalid file format, current file format: " + file_extension
                )
                with open(
                    "static\\"
                    + image_name.split(".")[0]
                ...
                file_name = (
                    image_name.split(".")[0] + "_" + jsonObj["Timestamp"] + ".json"
                )
            ...
            if img.format not in ["JPG", "JPEG", "PNG", "BMP", "TIFF"]:
                img_correct = corr_img_extn(
                    img, img_path, image_name, tempFilePaths, jsonObj
                )
                if img_correct:
                    img = img_correct
                else:
                    sql_dict.update(
                        {
                            "page_status": f"Invalid file format, current file format:{img.format}"
                        }
                    )
                    insert_log(sql_dict, request_id)
            ...
            img.close()
            siz_check = img_size_check(
                width, height, image_name, jsonObj, tempFilePaths
            )
            if not siz_check:
                sql_dict.update(
                    {
                        "page_status": f"Image is either too small in dimension or too large:Diam::{width,height}"
                    }
                )
                insert_log(sql_dict, request_id)
            ...
            jsonObj["Time"]["T_Time_Page_Image"] = str(
                datetime.strptime(
                    jsonObj["Time"]["Processed_Page_Image"], "%Y-%m-%d %H:%M:%S.%f"
                )
                - datetime.strptime(
                    jsonObj["Time"]["Started_Page_Image"], "%Y-%m-%d %H:%M:%S.%f"
                )
            )
            end_pg = datetime.now()
            append_csv(csv_row, end_pg)
        ...
        jsonObj["Time"]["Doc_time"] = str(
            datetime.strptime(jsonObj["Time"]["Final_response"], "%Y-%m-%d %H:%M:%S.%f")
            - datetime.strptime(
                jsonObj["Time"]["Process_Started"], "%Y-%m-%d %H:%M:%S.%f"
            )
        )
        with open(
            "static\\"
...
def img_process(
    ...
    try:
        cropped_image_path, crop_image_blob_url = crop_and_generate_image_url(
            new_image_name, csv_row, sql_dict, jsonObj
        )
        ...
        if not os.path.exists(new_img_path):
            ...
            for alt_ext in [".jpg", ".jpeg", ".png"]:
                ...
                if os.path.exists(alt_path):
                    debug_log(
                        f"Using alternative path for image: {alt_path}",
                        "img_process",
                        request_id,
                    )
        ...
        if not os.path.exists(new_img_path):
            raise FileNotFoundError(
                f"Image file not found at any extension: {new_img_path}"
            )
        ...
        if updated_img_cat == None and updated_txt_cat == None:
            image_data = get_cache_data(str(img_hash), request_id)
        else:
            delete_cache(str(img_hash), request_id)
            image_data = None
    except Exception as e:
        debug_log(
            f"Exception in checking cache as {str(e)} ", "img_process", request_id
        )
    ...
    if image_data != None:
        ...
        try:
            ...
            debug_log(
                f"Exception in loading image_schema as {str(e)} ",
                "img_process",
                request_id,
            )
        ...
        try:
            ...
            if updated_img_cat == None:
                ...
                sql_dict.update(
                    {
                        "class_of_page": new_cat,
                        "classification_time": end_cl_t - start_cl_t,
                    }
                )
            ...
            if new_cat in OCR_CATS:
                ...
                ocr_resp = Azure_ocr_sdk(
                    cropped_image_path, csv_row, sql_dict, request_id
                )
                if updated_txt_cat == None:
                    # new_cat = 'passport_front'
                    text_st = time.time()
                    new_cat = get_category_text(
                        ocr_resp["text_string"], new_cat, csv_row, sql_dict, request_id
                    )
                ...
                rotate_im_name = (
                    new_image_name.split(".")[0]
                    + "_cropped_rotated."
                    + new_image_name.split(".")[1]
                )
                rotate_im_dim = Image.open(rotate_im_path).size
                rot_coord_staus, rotated_cordinates = rotate_coordinates(
                    ocr_resp["words_loc"],
                    angle,
                    Image.open(cropped_image_path).size,
                    Image.open(rotate_im_path).size,
                    request_id,
                )
                if rot_coord_staus:
                    cropped_image, cropped_image_coordinates, cropped_text_blob = (
                        crop_img_text_locations(
                            Image.open(rotate_im_path),
                            rotate_im_path,
                            rotated_cordinates,
                            new_cat,
                            ocr_resp["text_string"],
                            request_id,
                        )
                    )
                else:
                ...
                rotated_cropped_img_name = (
                    rotate_im_name.split(".")[0]
                    + "_cropped."
                    + new_image_name.split(".")[1]
                )
                ...
                rotated_cropped_img_url = get_img_url_with_blob_sas_token(
                    rotated_cropped_img_name
                )
                ...
                jsonObj["Time"]["Azure_time"] = str(
                    datetime.strptime(
                        jsonObj["Time"]["Azure_complete"], "%Y-%m-%d %H:%M:%S.%f"
                    )
                    - datetime.strptime(
                        jsonObj["Time"]["Azure_called"], "%Y-%m-%d %H:%M:%S.%f"
                    )
                )
            ...
            sql_dict.update(
                {
                    "openai_time": "no_openai_called",
                    "face_api_time": "no_faceapi_called",
                }
            )
            ...
            image_schema = extract_data_based_on_doctype(
                image_schema,
                new_cat,
                cropped_image_coordinates,
                new_image_name,
            ...
            insert_cache(img_hash, image_schema, request_id)
            debug_log("inserted cache in db", "img_process", request_id)
            return image_schema
        except Exception as e:
            debug_log(
                f"Exception in img_process as {str(e)} ", "img_process", request_id
            )
            sql_dict.update({"page_status": "Failed"})
            sql_dict.update({"page_remark": str(e)})
            insert_log(sql_dict, request_id)
    ...
    try:
        ...
        if not val_api:
            face_sql_dict.update(
                {
                    "page_status": "Fail",
                    "page_remark": f"Please Provide a valid api key:: {api_key}",
                }
            )
        ...
        if not file_mimeType.startswith("image/"):
            face_sql_dict.update(
                {
                    "page_status": "Fail",
                    "page_remark": f"Not a image file {file_mimeType}",
                }
            )
            insert_face_log(face_sql_dict, None)
        ...
        if img.format not in ["JPG", "JPEG", "PNG", "BMP", "TIFF"]:
            ...
            if img_correct:
                ...
                face_sql_dict.update(
                    {
                        "page_status": "Fail",
                        "page_remark": f"Invalid file format, current file format: {img.format}",
                    }
                )
                insert_face_log(face_sql_dict, None)
                return "Invalid file format, current file format: " + img.format
        ...
        if not siz_check:
            face_sql_dict.update(
                {
                    "page_status": "Fail",
                    "page_remark": f"Image is either too small in dimension or too large {width,height}",
                }
            )
            insert_face_log(face_sql_dict, None)
            return "Image is either too small in dimension or too large. Reupload a right document"

        apiResponse = get_face_landmarks(
            uploaded_filename,
            img_path,
            image_name,
            customCrop,
...
def face_compare(filename1, file_content, filename2, file_content2, request_id):
    ...
    try:
        ...
        tempFilePaths = []
        img_path_1, image_name_1, file_extension_1 = save_and_gen_filepath(
            filename1, file_content, tempFilePaths
        )
        img_path_2, image_name_2, file_extension_2 = save_and_gen_filepath(
            filename2, file_content2, tempFilePaths
        )
        ...
        similarity_score, cropped_img1, cropped_img2 = cal_face_similarity(
            img_path_1, img_path_2, tempFilePaths
        )
        ...
        response.update(
            {
                "base64cropImg1": cropped_img1_string,
                "base64cropImg2": cropped_img2_string,
