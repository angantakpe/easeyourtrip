from sqlalchemy import create_engine, Column, String,JSON, Table, MetaData, DateTime, select , literal , insert , exists , delete
...
def insert_cache(image_hash, image_data):
    try:
        ...
        sel = select(literal(str(uuid.uuid4()))  , literal(str(image_hash)),literal(image_data_js),literal(datetime.now())).where(
            not_(exists().where(CACHE_TABLE.c.hash == str(image_hash)))  # Check if no row with id=1 exists
        )
...
def get_cache_data(image_hash):
    try:
        # get_cache_table()
        select_query = select(CACHE_TABLE.c.data).where(CACHE_TABLE.c.hash == str(image_hash))
        # print(f"select_query: {select_query}")
        with ENGINE.connect() as connection:
            result = connection.execute(select_query).fetchone()  # Fetch one matching row
...
def get_cache_table():
    ...
    try:
        if not sqlalchemy.inspect(ENGINE).has_table(Cache_table_name):  # If table doesn't exist, create it.
