...
@app.post('/process')
def classify_doc(file: Annotated[UploadFile,Form()],
...
@app.post('/validImage')
def validImage(file: Annotated[UploadFile,Form()],
    apikey: Annotated[str, Form()],  crop_img = Annotated[str, None] , custom_Crop = Annotated[str, None],height = Annotated[str, None] ,width = Annotated[str, None]):
    ...
    api_key = apikey
    result_final = process_photo(filename ,file_content, api_key , customCrop ,cropHeight , cropWidth, cropImg, jsonObj )
    return result_final

@app.post('/extract_with_category')
def upload_doc( apikey: Annotated[str, Form()],  file: Annotated[Optional[UploadFile], Form()] = None  , updated_cat: Annotated[Optional[str],Form()] =None, url: Annotated[Optional[str],Form()] =None):
    ...
    api_key = str(apikey)
    if api_key != os.environ.get('NURONAI_API_KEY') and api_key != os.environ.get('NURONAI_API_KEY2') and api_key != os.environ.get('NURONAI_API_KEY_LOCAL') :
    ...
    try: 
        response = docprocess(filename , file_content , str(api_key),updated_img_cat, updated_txt_cat , request_id)
        return response
    except Exception as e:    
